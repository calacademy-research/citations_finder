...
class KnownGoodPapers():
    ...
    def _associate_titles(self, start_year, end_year):
        ...
        for title in self.titles:
            ...
            if doi is not None:
                print(f"Doi {doi[0][0]} already in database associated to Title \"{title}\"")
                continue
            title = self.clean_html(title)
            doi = self.match_literal(title)
            if doi is not None:
                print("Literal match: ", end="")
            else:
                doi = self.match_fuzzy(title)
                if doi is not None:
                    print("Fuzzy match: ", end="")
            if doi is not None:
                print(f" {doi}: {title}")
        ...
        try:
            if doi is not None:
                if self._check_association_doi_exists(doi):
                    print(f"DOI {doi} already inserted, continuing...")
                    return
